...
7. **They add Products** — name, HSN code, unit, price, GST rate. This is their master data, so billing becomes a matter of picking from a list. A few conveniences here: typing an HSN code auto-suggests from a common quick-reference list and auto-fills the GST rate when it recognises the code (not exhaustive — see note below); the Unit field is a dropdown of standard GST unit codes (PCS, KGS, MTR, etc.) and remembers whichever one was used last, so repeat entries don't need reselecting it; and price can be entered either as the GST-inclusive final price or the excl.-GST base price — whichever one is typed, the other is calculated automatically from the selected rate.

    **Important note on the HSN reference list and GST rates generally:** the built-in HSN suggestions (`billing/assets/hsn-gst-reference.js`) cover common categories only — it is not the complete official rate schedule, which runs to thousands of entries and is revised periodically by GST Council notifications. The rates in this file reflect the GST 2.0 rate rationalisation effective 22 September 2025 (primary slabs simplified to 5%/18%, with a 40% slab for luxury/sin goods, and niche 0.25%/3% rates for gems). Always cross-check the [official CBIC rate finder](https://cbic-gst.gov.in/hindi/gst-goods-services-rates.html) for anything not in the list, or before filing if a rate seems uncertain — this tool speeds up data entry, it doesn't replace verifying your actual tax position.
