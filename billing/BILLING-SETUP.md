...
### Label-Based Auto Order: daily accumulation + Label Cropper handoff (new)

Two follow-up changes to `billing/buyer/label-order.html`, from real usage — a seller/buyer prints labels in batches throughout the day rather than all at once, and shouldn't have to send a tiny order after every batch:

- **The summary is now daily, not per-upload.** Each detected item merges into a running total in a new `users/{buyerUid}/labelOrderDrafts/{date}_{sellerId}` doc (owner-only, covered by the existing generic subcollection rule — no rules change needed) instead of an in-memory list that reset every time you processed a file. Upload a batch in the morning, another in the afternoon — "Today's Order Summary" keeps adding up across all of them, and survives closing the browser and coming back later the same day. Clicking **Send Order to [Seller]** creates the order from that day's accumulated total, then clears that seller's draft — so tomorrow (or the next batch today) starts fresh rather than double-counting.
- **The free Label Cropper tool can now hand its detected SKUs straight to this page.** `tools/label-cropper.html` has a **"Send to Label-Based Auto Order"** button next to its Detected SKUs panel — clicking it stores what was found in `sessionStorage` (same-origin, no server round trip) and opens this page, which picks it up automatically on load. No re-uploading the same PDF a second time. This is a one-click, explicit action the person takes — Label Cropper does **not** auto-redirect on its own, since it's a public tool used by plenty of people who have nothing to do with placing a VISUTRA order.
